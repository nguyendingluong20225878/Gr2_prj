    ...
    <Layout>
      <div className="space-y-6">
        <PageHeader
          ...
          description="Những token/khuyến nghị bạn đang theo dõi trước khi quyết định hành động."
        />
...
function getWatchReason(item: WatchItem) {
  const confidence = normalizeConfidenceValue(item.confidence);
  if (item.status === 'RESOLVED') return 'Đã được đánh dấu có kết quả';
  if (item.status === 'EXPIRED') return 'Đã hết hiệu lực theo watchlist';
  if (item.proposal) return 'Đang theo dõi từ khuyến nghị còn hiệu lực';
  if (confidence !== null && confidence >= 70) return 'Đáng theo dõi vì độ tin cậy cao';
